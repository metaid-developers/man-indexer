...
type Db interface {
	...
	GetMrc20TickPageList(cursor int64, size int64, order string) (total int64, list []mrc20.Mrc20DeployInfo, err error)
	AddMrc20Shovel(shovelList []string, pinId string) (err error)
	...
	GetHistoryByAddress(tickId string, address string, cursor int64, size int64) (list []mrc20.Mrc20Utxo, total int64, err error)
	GetMrc20BalanceByAddress(address string) (list []mrc20.Mrc20Balance, total int64, err error)
	GetHistoryByTx(txId string, index int64, cursor int64, size int64) (list []mrc20.Mrc20Utxo, total int64, err error)
	GetShovelListByAddress(address string, lv int, path, query, key, operator, value string, cursor int64, size int64) (list []*pin.PinInscription, total int64, err error)
}
