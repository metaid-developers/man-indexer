...
type Db interface {
	...
	UpdateTransferPin(addressMap map[string]string) (err error)
	AddMempoolPin(pin *pin.PinInscription) (err error)
	GetRootTxId(address string) (metaId string, err error)
	GetMetaIdInfo(rootTxid string, key string) (info *pin.MetaIdInfo, unconfirmed string, err error)
	...
	GetPinRootByAddress(address string) (pin *pin.PinInscription, err error)
	...
	GetMetaIdPin(roottxid string, page int64, size int64) (pins []*pin.PinInscription, total int64, err error)
	Count() (count pin.PinCount)
	GetChildNodeById(roottxid string) (pins []*pin.PinInscription, err error)
