...
type Db interface {
	...
	GetMrc20TickPageList(cursor int64, size int64, order string, completed string) (total int64, list []mrc20.Mrc20DeployInfo, err error)
	...
	GetShovelListByAddress(address string, creator string, lv int, path, query, key, operator, value string, cursor int64, size int64) (list []*pin.PinInscription, total int64, err error)
}
